...
module.exports.run = async function({ api, event, args }) {
    ...
    if (!query) {
        api.sendMessage('Please provide a question ex: ai what love', event.threadID, event.messageID);
        return;
    }

    const initialMessage = await api.sendMessage('Searching please wait.....', event.threadID, event.messageID);
    ...
    try {
        ...
        api.getUserInfo(event.senderID, (err, result) => {
            ...
            api.sendMessage(finalResponse, event.threadID, (err, messageInfo) => {
                if (err) {
                    console.error('Error sending message:', err);
                } else {
                    // Edit the initial message to the final response
                    api.editMessage(finalResponse, messageInfo.messageID, (err) => {
                        if (err) {
